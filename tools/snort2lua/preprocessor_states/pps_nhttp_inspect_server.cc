...
namespace
{
class NHttpInspectServer : public ConversionState
{
public:
    NHttpInspectServer(Converter& c) : ConversionState(c) { }
    ~NHttpInspectServer() { }
...
int NHttpInspectServer::binding_id = 0;

bool NHttpInspectServer::convert(std::istringstream& data_stream)
{
    std::string keyword;
    bool retval = true;
    ...
    while (data_stream >> keyword)
    {
        ...
        else if (!keyword.compare("client_flow_depth"))
        {
            ...
            parse_deleted_option("oversize_dir_length", data_stream);
...
{
    return new NHttpInspectServer(c);
}
