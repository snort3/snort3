...
static inline Stream5Config* GetS5Config(SnortConfig*)
{
    return (Stream5Config*)get_inspection_policy()->s5_config;
}
...
void Stream::init_active_response(Packet* p, Flow* flow)
{
    ...
    Stream5GlobalConfig* gc = flow->s5_config->global_config;
    flow->response_count = 1;

    if ( gc->max_active_responses > 1 )
        flow->set_expire(p, gc->min_response_seconds);
}
...
bool Stream::register_paf_port(
    SnortConfig* sc, uint16_t server_port, bool to_server,
    PAF_Callback cb, bool autoEnable)
{
    return s5_paf_register_port(sc, server_port, to_server, cb, autoEnable);
}
...
{
    Stream5Config* config = lwssn->s5_config;
...
void Stream::set_service_filter_status (
    SnortConfig*, int protocolId, int status)
{               
    Stream5Config* config = GetS5Config(snort_conf);
...
int Stream::get_service_filter_status ( 
    SnortConfig*, int protocolId)
{       
    Stream5Config* config = GetS5Config(snort_conf);
                
    if ( !config )
        return 0;  // FIXIT default?
                            
    return config->service_filter[protocolId];
}       

void Stream::set_port_filter_status(
    SnortConfig* sc,int protocol, uint16_t port, uint16_t status
        )
{
    Stream5Config* s5 = GetS5Config(sc);
    ...
    switch (protocol)
    {
        case IPPROTO_TCP:
            if ( s5->tcp_config )
                s5TcpSetPortFilterStatus(s5->tcp_config, port, status);
            break;
        case IPPROTO_UDP:
            if ( s5->tcp_config )
                s5UdpSetPortFilterStatus(s5->udp_config, port, status);
            break;
    }
}

void Stream::unset_port_filter_status(
    SnortConfig* sc,int protocol, uint16_t port, uint16_t status)
{
    ...
    switch (protocol)
    {
        case IPPROTO_TCP:
            if ( s5->tcp_config )
                s5TcpUnsetPortFilterStatus(s5->tcp_config, port, status);
            break;
        case IPPROTO_UDP:
            if ( s5->udp_config )
                s5UdpUnsetPortFilterStatus(s5->udp_config, port, status);
            break;
    }
}

bool Stream::get_filter_status(Stream5Config* s5, Packet* p)
{
    int protocolId = GetProtocolReference(p);

    if ((protocolId > 0) && get_service_filter_status(snort_conf, protocolId))
    {
        return false;
    }

    uint8_t  action = 0;
    bool ignore_any_rules = false;
    tPortFilterStats* pPortFilterStats = NULL;

    int proto = GET_IPH_PROTO(p);
    ...
    switch ( proto )
    {
        case IPPROTO_TCP:
            if ( s5->tcp_config )
            {
                action = s5TcpGetPortFilterStatus(s5->tcp_config, p->sp) |
                    s5TcpGetPortFilterStatus(s5->tcp_config, p->dp);

                ignore_any_rules = s5TcpIgnoreAny(s5->tcp_config);
            }
        ...
        case IPPROTO_UDP:
            if ( s5->udp_config )
            {
                action = s5UdpGetPortFilterStatus(s5->udp_config, p->sp) |
                    s5UdpGetPortFilterStatus(s5->udp_config, p->dp);

                ignore_any_rules = s5UdpIgnoreAny(s5->udp_config);
            }
    ...
    if (!(action & PORT_MONITOR_SESSION_BITS))
    {
        ...
        {
            DisableDetect(p); /* Ignore this TCP packet entirely */
        ...
        UpdateFilteredPacketStats(&sfBase, proto);
        return true;
    }
...
int Stream::alert_flush_stream(Packet *p)
{
    Flow* flow;
    ...
    if (!(flow->s5_config->global_config->flags & STREAM5_CONFIG_FLUSH_ON_ALERT))
    {
        DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE,
