...
static PAF_Config* get_config (SnortConfig*)
{
    InspectionPolicy* sp;
#ifdef POLICY_BY_ID_ONLY
    sp = get_default_inspection_policy();
#else
    sp = get_inspection_policy();
#endif
    assert(sp);

    Stream5Config* config = (Stream5Config*)sp->s5_config;

    if ( !config || !config->tcp_config )
        return NULL;

    return (PAF_Config*)get_paf_config(config->tcp_config);
}
...
bool s5_paf_register_port (
    SnortConfig* sc, uint16_t port, bool c2s, PAF_Callback cb, bool auto_on)
{
    PAF_Config* pc = get_config(sc);
    int i, dir = c2s ? 1 : 0;

    if ( !pc )
        return false;

    i = stream.set_paf_callback(cb);
...
bool s5_paf_register_service (
    SnortConfig* sc,uint16_t service, bool c2s, PAF_Callback cb, bool auto_on)
{
    PAF_Config* pc = get_config(sc);
    int i, dir = c2s ? 1 : 0;
