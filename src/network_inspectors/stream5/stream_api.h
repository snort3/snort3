...
#ifdef ENABLE_HA
typedef uint32_t (*StreamHAProducerFunc)(Flow*, uint8_t *buf);
typedef int (*StreamHAConsumerFunc)(Flow*, const uint8_t *data, uint8_t length);
#endif
...
public:
     ...
     */
    static int alert_flush_stream(Packet*);  // PKT
     ...
     */
    static void set_service_filter_status(
        SnortConfig* sc, int service, int status);
    static int get_service_filter_status(SnortConfig* sc, int service);
     ...
     *  sure to set the parsing argument to 1.
     */
    static void set_port_filter_status(
     ...
     */
    static void unset_port_filter_status(
        SnortConfig* sc, int protocol, uint16_t port, uint16_t status);
    static bool get_filter_status(Stream5Config*, Packet*);
    ...
    static bool register_paf_port(
            SnortConfig*, uint16_t server_port, bool toServer,
     ...
     *      < 0 on failure
     */
    static int register_ha_funcs(
        uint32_t preproc_id, uint8_t subcode, uint8_t size,
        StreamHAProducerFunc produce, StreamHAConsumerFunc consume);

    /* Indicate a pending high availability update for a given session.
     */
    static void set_ha_pending_bit(Flow*, int haPendingUpdateBit);
#endif
