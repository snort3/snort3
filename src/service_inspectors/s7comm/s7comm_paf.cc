...
#define S7COMM_MIN_HDR_LEN 10  // Minimum length for S7comm header

S7commSplitter::S7commSplitter(bool b) : StreamSplitter(b)
{
    state = S7COMM_PAF_STATE__TPKT_VER;
    tpkt_length = 0;
}
...
StreamSplitter::Status S7commSplitter::scan(
    Packet*, const uint8_t* data, uint32_t len, uint32_t /*flags*/, uint32_t* fp)
{
    ...
    while (bytes_processed < len)
    {
        ...
        case S7COMM_PAF_STATE__TPKT_VER:
        case S7COMM_PAF_STATE__TPKT_RESERVED:
        case S7COMM_PAF_STATE__COTP_LEN:
        case S7COMM_PAF_STATE__COTP_PDU_TYPE:
            state = (s7comm_paf_state_t)(((int)state) + 1);     // Set the state to next PAF state
            break;

        /* Read length 1 byte at a time, in case a TCP segment is sent
         * with xxx bytes from the S7CPAP header */
        case S7COMM_PAF_STATE__TPKT_LEN_1:
            tpkt_length |= ( *(data + bytes_processed) << 8 );
            state = S7COMM_PAF_STATE__TPKT_LEN_2;
            break;

        case S7COMM_PAF_STATE__TPKT_LEN_2:
            tpkt_length |= *(data + bytes_processed);
            state = S7COMM_PAF_STATE__COTP_LEN;
            break;

        case S7COMM_PAF_STATE__SET_FLUSH:
            if ((tpkt_length < S7COMM_MIN_HDR_LEN))
            {
