...
//-------------------------------------------------------------------------
// uricontent api methods
//-------------------------------------------------------------------------
// FIXIT split into 3 files: content, uricontent, and pmd

static IpsOption* uricontent_ctor(
    SnortConfig* sc, char *data, OptTreeNode * otn){
    PatternMatchData *pmd;
    char *data_end;
    char *data_dup;
    ...
    if (!data)
        ParseError("No uricontent pattern specified!");
    ...
    while (next_opt < data_end)
    {
        ...
        {
            ParseError("Invalid uricontent parameter specified: %s", opts[0]);
        }
        mSplitFree(&opts, num_opts);
    }

    free(data_dup);
    ValidateContent(sc, pmd, RULE_OPTION_TYPE_CONTENT_URI);

    return new UriContentOption(pmd);
}

static void uricontent_dtor(IpsOption* p)
{
    delete p;
}

static void uricontent_ginit(SnortConfig*)
{
#ifdef PERF_PROFILING
    RegisterPreprocessorProfile(
        "uricontent", &uricontentPerfStats, 3, &ruleOTNEvalPerfStats, con_get_profile);
#endif
}

static const IpsApi uricontent_api =
{
    {
        PT_IPS_OPTION,
        "uricontent",
...
const BaseApi* ips_uricontent = &uricontent_api.base;
