...
static bool is_stub_option(std::string& opt)
{
    if ( s_plain )
        return true;
...
static bool parse_so_rule(const char* in, bool plain, std::string& stub, std::string& opts)
{
    s_stub = true;
    s_plain = plain;
...
{
    std::string opts;
    return parse_so_rule(in, plain, stub, opts);
}
...
TEST_CASE("parse_so_rule", "[parser]")
{
    ...
    while ( tc->rule )
    {
        std::string stub;
        std::string opts;
        bool parse = parse_so_rule(tc->rule, false, stub, opts);
